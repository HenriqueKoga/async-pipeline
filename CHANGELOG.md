...
- Added improved typed context support
- Added support for dataclass/object contexts
- Added typed context example (`examples/typed_context.py`)

### Changed

- Improved context handling in middlewares (dict-like and attribute-based contexts)
- Preserved backward compatibility with dict contexts
...
- Built-in middlewares package (`async_pipeline.middlewares`)
- `LoggingMiddleware`, `TimingMiddleware`, `RetryMiddleware`, `TimeoutMiddleware`
- Documented middleware ordering (list order = outermost first)
